...
> **Amended 2026-08-23 (closure).** FW-13 closed on `feat/fw-13-ws-client-hello` against `main@58f387e` (post-FW-11 merge PR #12) — 13 work-unit commits land; single PR with `size:exception` per preflight (1000-line review budget + extend-if-needed; final ~3,950 LoC across 44 files). Closes R-07 (WS client initialised + started, lazy on IP_EVENT_STA_GOT_IP), R-09 (MAC = canonical identity; read from eFuse via `esp_read_mac`, never stored in NVS), R-17 (WS pipeline config: TCP transport, no MAC in URL path, ping_interval=10 s, pingpong_timeout=30 s, buffer_size=16384, network_timeout=5000 ms, task_stack=8192, `disable_auto_reconnect=true` so FW-14 owns reconnect), R-27 (protocol contract outbound frames: hello on CONNECTED + status every 30 s). Unblocks FW-14 (WS reconnect), FW-15 (stream task → WS binary), FW-18 (control dispatcher), FW-20 (camera re-init emits fresh hello via status).
>
> **Work-unit commit ledger** (13 commits; SHAs from `feat/fw-13-ws-client-hello`):
>
> | # | Commit SHA | Title | What | Closes |
> |---|---|---|---|---|
...
>
> **Verify-report verdict**: PENDING — full 11-pass runner green + `idf.py build` green. Device-flash end-to-end verify (boot → wifi join → IP-up → WS CONNECTED → hello emit → status cadence) is the user's responsibility per T-13-K PR body caveat (no backend on `ws://<host>:<port>/cams` available in this orchestrator session).
>
> **Known deviations from design** (resolved with documentation, not silent):
>
...
> 8. PR LoC ~3,950 (vs pre-approved 1000-line budget + extend-if-needed) — covered by user's `exception-ok` preflight qualifier. The work is tightly coupled per FW-13 Notes L1186 ("hello and status share the same text-frame builder and identity source") so a single PR is the documented choice.
>
> **Device-flash verify deferred to next session**: the AI-Thinker ESP32-CAM at `/dev/cu.usbserial-130` was not flashed in this session (no backend running on `ws://<host>:<port>/cams` to receive the hello + status frames; no orchestrator-side flash capability). The PR body carries the "device-verified end-to-end pending — user must `idf.py flash` + connect to a backend" caveat per the FW-05/08/10/11 precedent. The firmware.bin produced by `idf.py build` IS flash-ready.
>
