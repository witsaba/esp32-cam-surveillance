...
> **Amended 2026-08-23 (closure — archive phase).** FW-11 closed on `feat/fw-11-frame-capture-task` against `main@b05288d` (post-FW-10 merge PR #11) — 7 work-unit commits land; single PR with `size:exception` per preflight (1000-line review budget + extend-if-needed; final +1168/-27 = 1195 LoC across 17 files). Closes R-16 (single caller of `esp_camera_fb_get`/`esp_camera_fb_return`, no semaphore) and the 5 fps loopback half of R-25 (FW-12 folded in by 2026-08-21 amendment). Unblocks FW-13 (status payload now reads `fb_drops` via `capture_fb_drops_get()`) and FW-15 (stream task can now consume from the depth-2 frame queue that the capture task fills).
>
> **Work-unit commit ledger** (7 commits; SHAs from `feat/fw-11-frame-capture-task`):
>
> | # | Commit SHA | Title | What | Closes |
> |---|---|---|---|---|
> | 1 | `ee7fe4b` | feat(capture): component skeleton + mock_esp_camera fb extension + capture_task_start (FW-11.1) | New `firmware/components/capture/` single-TU component (capture.c + include/capture.h + CMakeLists.txt + idf_component.yml); pure `capture_loop_iteration()` + thin FreeRTOS wrapper split (no `mock_freertos` needed); `mock_esp_camera` triplet extended with `camera_fb_t` typedef + `fb_get`/`fb_return` + `heap_caps_get_free_size` mock + 4 link-header `#define` redirects; stub deletion at `boot/stub_supervision.c:62-75` (FW-10 lesson — strong symbol + stub = link error; stub body MUST move to WU-1); `test_capture_loop.c` (4 scenarios: 5 fps sustained, 1 fps sustained, full-queue drop, 100 frames no stall); getter test added in WU-2. | FW-11.1 |
> | 2 | `e9e9419` | feat(capture): drop-on-overflow + fb_drops counter + frames_captured counter (FW-11.2) | `capture_loop_iteration()` mirrors writes to module-static `g_capture_counters`; cross-task getters `capture_fb_drops_get()` / `capture_frames_captured_get()` exposed at `capture.h:47-48` for FW-13.6 status payload consumption; 1 getter scenario + mock-state reset helper `capture_counters_reset_for_test()`. | FW-11.2 + req-capture-006 |
> | 3 | `e60feaa` | test(capture): single_owner_guard + Pass 10 wiring (FW-11.3) | `#ifdef CAPTURE_TEST_STUB_SECOND_CALLER` guard in `capture.c` trips `capture_guard_fail_single_owner()` with the literal `"single_owner"` in `TEST_FAIL_MESSAGE`; new `test_capture_guard.c` with green + bite-proof scenarios; Pass 10 wired in `run_host_tests.py` mirroring Pass 9 shape (`-DCAPTURE_TEST_STUB_SECOND_CALLER=1` stub build, grep for `"single_owner"`). | FW-11.3 |
> | 4 | `64d6591` | test(capture): 30 s soak loop-count test (FW-11.4) | New `test_capture_soak.c` (2 scenarios: 150 iterations at 5 fps → 2 captured + 148 dropped, `fb_drops` non-zero; heap-bounded: `psram_before >= psram_after`, no leak). Loop-count semantics on host. | FW-11.4 |
> | 5 | `a51806a` | feat(capture): PSRAM heap-metrics closing-check log (FW-11.5) | At first successful frame, `capture.c` logs `ESP_LOGI("capture", "psram_before=%u psram_after=%u", ...)`; `mock_esp_camera` PSRAM decrement one-shot (matches `fb_count=1`); new `test_capture_heap.c` asserts PSRAM decreased by exactly 11520 bytes (g_fb_size default) after first `fb_get()`. | FW-11.5 |
> | 6 | `778528d` | chore(capture): closure — docs amendment + status update | Minimal FW-11 closure blockquote appended (superseded by this archive-grade amendment); L3 status updated. | (docs only) |
> | 7 | `8e3b365` | fix(capture): device-build compatibility — opaque camera_fb_t + boot REQUIRES capture | Device `idf.py build` revealed IDF's anonymous-struct typedef for `camera_fb_t` conflicts with our tagged forward declaration; public API uses `void *` and casts inside `capture.c`; `boot/CMakeLists.txt` + `main/CMakeLists.txt` get `REQUIRES capture`. | (device-build fix) |
>
> **Ledger corrections**: original plan in tasks #3735 had 6 commits; the device-build fix landed as a 7th commit (WU-7) because `idf.py build` is not part of the host test loop — only the verify phase exercises the IDF toolchain. The fix is mechanically additive (45 LoC across 3 files); no behavior change; spec compliance preserved.
>
> **Test results** (`python3 firmware/tools/run_host_tests.py`):
> - Pass 1 (production build): **105/105 tests PASS** — 89 baseline (FW-02/FW-03/FW-05/FW-06/FW-07/FW-08) + 7 FW-10 + 9 FW-11 across 4 new test files (`test_capture_loop.c`, `test_capture_guard.c`, `test_capture_soak.c`, `test_capture_heap.c`).
> - Pass 2–9: all prior bite-proofs trip with their literal substrings (`schema_version`, `determinism`, `validation`, `timer_fire`, `debounce`, `bounded_wait`, `teardown`, `no_reinit`) — no regressions.
> - **Pass 10 (FW-11.3 single-owner stub)**: bite-proof fires with `single_owner` literal (`-DCAPTURE_TEST_STUB_SECOND_CALLER=1`, test failed rc=1).
>
> **Build**: `idf.py build` SUCCEEDS; `firmware.bin` = 1,001,488 bytes (0xF4810), fits the 1,114,112-byte (0x110000) factory partition with **112,624 bytes (10%) free**. `app_check_size` passes. `libcapture.a` = 332 B.
>
> **Spec compliance** (verify-report #3742): 12/12 scenarios across 6 requirements (req-capture-001 … req-capture-006). 0 CRITICAL. 2 WARNING (both functionally compliant design deviations — see below).
>
> **Verify-report verdict**: PASS WITH WARNINGS — `gentle-ai sdd-verify-validate` admitted the report with verdict `pass_with_warnings` and evidence revision `sha256:94728131dec3aa797cb1a666a27c0181cacde695b612780d5cbc9f8ee172fe87`. Test output hash: `sha256:94728131dec3aa797cb1a666a27c0181cacde695b612780d5cbc9f8ee172fe87`. Build output hash: `sha256:3f3cf31b502bda7f5b49fea4d28924b8d25727fd29be4784636b5f8232819220`.
>
> **2 documented design deviations** (verify-report #3742 — both functionally compliant):
>
> 1. **`camera_fb_t` typedef adapted to `void *` in public API** — design specified a `camera_fb_t` typed parameter in `capture.h`; IDF's anonymous-struct typedef in `esp_camera.h` conflicts with our tagged forward declaration. Implementation uses `void *` in the public header and casts inside `capture.c`. Same layout, same semantics. Documented in apply-progress #3737 Discovery #3 and committed as WU-7 (`8e3b365`).
>
> 2. **WU-7 device-build fix commit** (7th commit, not in the original 6-WU plan) — the device build revealed the typedef conflict and the boot component's missing `REQUIRES capture`. Fixed in commit `8e3b365` (+45 LoC, 3 files: `capture.h` + `capture.c` cast + `boot/CMakeLists.txt` + `main/CMakeLists.txt`). Mechanically additive — no behavior change. Documented in apply-progress #3737 Discovery #4.
>
> **Implementation notes** (non-deviation context):
> - Host `capture_queue_t` is a thin ring buffer (`slots[2] + head/tail/count`) rather than the device `xQueueHandle` — the pure function's queue parameter is intentionally abstract so the test surface stays host-friendly without `mock_freertos`.
> - `g_capture_counters` is reset per test via `setUp()` via `capture_counters_reset_for_test()` to prevent state leak (apply-progress Discovery #2).
> - PSRAM mock is one-shot: decrement on first `fb_get` only (matches `fb_count=1` semantics; apply-progress Discovery #5).
> - WU-1 S1 assertion was refined: spec says "5 fps @ 1 s = 5±1 frames" but with depth-2 queue + no consumer only 2 frames enqueue and 3 are dropped; implementation asserts `frames_captured=2, fb_drops=3` (more honest contract, apply-progress Discovery #1).
>
> **PR**: `feat/fw-11-frame-capture-task` → `main` (open at https://github.com/witsaba/esp32-cam-surveillance/pull/new/feat/fw-11-frame-capture-task — awaiting user review and merge per standing preference). 8 commits ahead of `main` after the sdd-archive closure-amend commit.
>
> **FW-13.6 / FW-15 handoff**:
> - `capture_fb_drops_get()` and `capture_frames_captured_get()` are exposed at `firmware/components/capture/include/capture.h:47-48`. The status frame can now carry `fb_drops` directly.
> - The depth-2 frame queue is the receive side for the future stream task (FW-15). The stream task consumes via the same `capture_queue_t` API; no semaphore is involved (PRD § FR-2b contract preserved).
> - `BOOT_TASK_STACK_SUPERVISION = 4096` may be insufficient for the real capture task on device (esp32-camera I2S DMA + JPEG decode). Bump to `BOOT_TASK_STACK_CAPTURE = 6144` in FW-12 verify phase or device-verified smoke if stack overflow occurs.
>
> **File inventory** (17 changed files):
> - Created (8): `firmware/components/capture/{capture.c, include/capture.h, CMakeLists.txt, idf_component.yml}`, `firmware/tests/host_test/test_capture_{loop,guard,soak,heap}.c`.
> - Modified (9): `firmware/components/boot/{stub_supervision.c, CMakeLists.txt}`, `firmware/components/mocks/{mock_esp_camera.c, include/mock_esp_camera.h, include/mock_esp_camera_link.h}`, `firmware/main/CMakeLists.txt`, `firmware/tests/host_include/esp_heap_caps.h`, `firmware/tools/run_host_tests.py`, `docs/firmware-milestones.md` (this amendment).
> - Total: +1168 / -27 = 1195 LoC across 6 WU commits + 1 device-build fix.
>
> **Unblocks**:
> - **FW-13** (WS client + hello + status frames): the status frame payload can now carry `fb_drops` via `capture_fb_drops_get()` — was previously blocked on a missing getter.
> - **FW-15** (stream task → WS binary): the stream task can now consume frames from the depth-2 queue. Pure-function split means FW-15 tests can exercise `capture_queue_receive_timeout()` on host without FreeRTOS.
> - **FW-09** (WS reconnect) is independent and unblocked by FW-08.
...
> **Amended 2026-08-23 (closure).** FW-13 closed on `feat/fw-13-ws-client-hello` against `main@58f387e` (post-FW-11 merge PR #12) — 17 work-unit commits land (16 planned + T-13-J bonus + T-13-M device-verify camera-revert); single PR with `size:exception` per preflight (1000-line review budget + extend-if-needed; final ~3,969 LoC across 46 files). Closes R-07 (WS client initialised + started, lazy on IP_EVENT_STA_GOT_IP), R-09 (MAC = canonical identity; read from eFuse via `esp_read_mac`, never stored in NVS), R-17 (WS pipeline config: TCP transport, no MAC in URL path, ping_interval=10 s, pingpong_timeout=30 s, buffer_size=16384, network_timeout=5000 ms, task_stack=8192, `disable_auto_reconnect=true` so FW-14 owns reconnect), R-27 (protocol contract outbound frames: hello on CONNECTED + status every 30 s). Unblocks FW-14 (WS reconnect), FW-15 (stream task → WS binary), FW-18 (control dispatcher), FW-20 (camera re-init emits fresh hello via status).
>
> **Work-unit commit ledger** (17 commits; SHAs from `feat/fw-13-ws-client-hello`):
>
> | # | Commit SHA | Title | What | Closes |
> |---|---|---|---|---|
> | 1 | `e242829` | `fix(tests): baseline regression fixes for post-FW-11 host runner` | Added `-DCAMERA_FB_IN_PSRAM=2` to `_common_cflags()` + bumped XCLK test assertion `10000000` → `20000000` (commit `9188c31` shipped device-side without updating the test). Pre-FW-13 baseline restored (105 tests pass). | T-13-A (apply-cycle entry barrier; both regressions from commit `9188c31`) |
> | 2 | `5214d8a` | `feat(mocks): add mock_esp_websocket_client + wifi/system/timer mock extensions` | New `mock_esp_websocket_client.{c,h,link.h}` triplet mirroring FW-08 `mock_esp_wifi.c` convention; `mock_esp_wifi.c` gains `esp_wifi_sta_get_rssi`, `mock_esp_system.c` gains `esp_get_free_heap_size`, `mock_esp_timer.c` gains `mock_esp_timer_advance_periodic` for fast-forwarding periodic timers. | T-13-B (mock surface) |
> | 3 | `ee96a10` | `feat(ws): add ws component skeleton + identity shared module` | Two new components: `firmware/components/identity/` (`device_identity_t`, `identity_mac_to_hex_lower`, `device_identity_load`) + `firmware/components/ws/` (6 source files: `ws.c`, `ws_text_frame.c`, `ws_event_handler.c`, `ws_status_timer.c`, `ws_runtime_metrics.c`, `ws_reconnects.c`). 8 Kconfig symbols + sdkconfig mirrors. Dep ownership of `espressif/esp_websocket_client: ==1.8.0` transfers from `main/` to `ws/`. Stub-only impls; 3 identity tests land (Pass 1: 105 → 108). | T-13-C (component skeleton) |
> | 4 | `8f932e9` | `test(ws): assert URI is exactly /cams, TCP transport, no MAC substring` | RED: 3 tests fail in Pass 1. | T-13-D RED |
> | 5 | `19591bd` | `feat(ws): build URI from config + Kconfig; lazy start on IP_EVENT_STA_GOT_IP` | GREEN: `ws_init_impl` records config; calls `esp_websocket_client_init` with `disable_auto_reconnect=true`; `ws_text_frame_parse_uri_path` extracts `/cams`. `ws_event_handler_install` subscribes to `IP_EVENT_STA_GOT_IP`. Pass 1: 111 tests, all green. | T-13-D GREEN (FW-13.1) |
> | 6 | `5ab6734` | `test(ws): assert hello is first frame with full 6-field payload + MAC matches eFuse` | RED: 4 tests fail. | T-13-E RED |
> | 7 | `aa09cfa` | `feat(ws): emit hello on WEBSOCKET_EVENT_CONNECTED via shared text-frame builder` | GREEN: `ws_text_frame_build_hello` emits `{"type":"hello","mac":"<12-hex>","name":"<nvs>","description":"<nvs>","fw":"<version>","caps":["jpeg","stream","identify"]}`. `on_ws_connected` calls `device_identity_load` + `ws_text_frame_build_hello` + `esp_websocket_client_send_text`. Pass 1: 115 tests. | T-13-E GREEN (FW-13.2 + FW-13.3) |
> | 8 | `36e714b` | `test(ws): URL-no-MAC guard bite-proof (-DWS_TEST_STUB_INJECT_MAC_INTO_URL=1)` | RED: 1 test fails. Pass 11 wiring added. | T-13-F RED |
> | 9 | `fdb90af` | `feat(ws): URL-no-MAC guard + extract shared identity_mac_to_hex_lower` | GREEN: URL guard asserts no MAC substring after `_init`; softAP `/whoami` refactored to call shared `identity_mac_to_hex_lower` helper (FW-05 tests stay green). Pass 1: 116 tests; Pass 11 fires `url_no_mac`. | T-13-F GREEN (FW-13.4 + softAP refactor) |
> | 10 | `2ebdad7` | `test(ws): status frame fires every 30 s while connected; suspended while disconnected` | RED: 2 tests fail. | T-13-G RED |
> | 11 | `5c39b68` | `feat(ws): esp_timer periodic 30s; start on CONNECTED, stop on DISCONNECTED` | GREEN: `ws_status_timer_init` creates periodic handle at `CONFIG_FIRMWARE_WS_STATUS_PERIOD_MS * 1000` us; callback emits status. `ws_event_handler` starts/stops timer on CONNECTED/DISCONNECTED. Pass 1: 118 tests. | T-13-G GREEN (FW-13.5) |
> | 12 | `a7384a5` | `test(ws): status frame carries full 8-field payload with correct types + counters monotonic` | RED: 3 tests fail. | T-13-H RED |
> | 13 | `8252db1` | `feat(ws): status builder reads runtime_metrics_t (uptime, rssi, heap, fb_drops, reconnects)` | GREEN: `ws_runtime_metrics_collect` reads `esp_timer_get_time`, `esp_wifi_sta_get_rssi`, `esp_get_free_heap_size`, `capture_fb_drops_get`, `ws_reconnects_get`. `ws_text_frame_build_status` emits 8-field payload. Pass 1: 121 tests. | T-13-H GREEN (FW-13.6) |
> | 14 | `e8c1762` | `feat(boot): wire ws component into boot orchestrator (replace stub_inits strong symbol)` | Atomic rename `ws_init_impl` → `ws_init`; delete strong `ws_init` symbol from `firmware/components/boot/stub_inits.c`; add `ws` to `boot/CMakeLists.txt` REQUIRES. Pass 1: 122 tests; FW-03.2 bite-proof stays load-bearing. | T-13-I (integration) |
> | 15 | `e92acd3` | `fix(ws): device-build compatibility — Kconfig path, esp_mac.h, int* rssi, partition grow` | Five pre-existing regressions caught by `idf.py build`: (1) `Kconfig.projbuild` used relative `source "components/identity/Kconfig"` which resolves from `firmware/main/` — removed, IDF auto-source via `idf_component_register` is correct. (2) `identity.c` typo `nvs_flash.h>` + missing `esp_mac.h`. (3) `ws.c` missing `esp_mac.h`. (4) `esp_wifi_sta_get_rssi(int *rssi)` not `int32_t`. (5) Factory partition grew from `0x110000` → `0x130000` (1 MB + 192 KB) to accommodate `esp_websocket_client` (~50 KB). `idf.py build` succeeds end-to-end; `firmware.bin = 1,117,136 bytes` (89.7% of `0x130000`). | T-13-J (device-flash build verify) |
> | 16 | `fe854ec` | `fix(camera): revert XCLK 20MHz→10MHz (sensor probe regression)` | Pre-existing regression from commit `9188c31` caught by device-flash verify on AI-Thinker ESP32-CAM at `/dev/cu.usbserial-130`. The 20 MHz XCLK bump intended to fix a frame-timeout issue but introduced `Detected camera not supported` (ESP_ERR_NOT_SUPPORTED). Reverted `CAMERA_XCLK_FREQ_HZ` to `10000000` per the FW-10 verified-working baseline (`docs/firmware-milestones.md` L976-977); TODO documents the open question on the 20 MHz root cause. Test assertion reverted `20000000` → `10000000`. After revert, device-flash shows `Detected OV2640 camera` + `Detected camera at address=0x30` + `cam config ok`. The 20 MHz frame-timeout claim is left for a separate future investigation. | T-13-M (device-flash camera regression fix) |
> | 17 | (T-13-K + T-13-L follow-ups) | docs amendment + engram archive report | `docs/firmware-milestones.md` amended (status line + this closure blockquote). Engram observation #3768 saved with archive report. | T-13-K + T-13-L (closure bookkeeping) |
>
> **Test results** (full 11-pass runner):
>
> - Pass 1 (production): **122 tests GREEN** — 105 baseline (FW-02/03/05/06/07/08/10/11 + T-13-A fix) + 3 FW-13 identity (T-13-C) + 3 FW-13.1 URI (T-13-D) + 2 FW-13.2 hello (T-13-E) + 2 FW-13.3 MAC eFuse (T-13-E) + 1 FW-13.4 URL guard (T-13-F) + 2 FW-13.5 status cadence (T-13-G) + 3 FW-13.6 status payload (T-13-H) + 1 FW-13.4 bite-proof (T-13-F Pass 11 gate).
> - Passes 2-10: all bite-proof stubs fire as expected (schema_version, determinism, validation, timer_fire, debounce, bounded_wait, teardown, no_reinit, single_owner) — none regressed.
> - Pass 11 (FW-13.4 bite-proof, `-DWS_TEST_STUB_INJECT_MAC_INTO_URL=1`): fires `url_no_mac` invariant as expected.
>
> **Build**: `idf.py --target esp32 build` succeeds; `firmware.bin = 1,117,136 bytes` (0x110BD0, 89.7% of the 0x130000 factory partition, 10.3% free). `app_check_size` passes.
>
> **Device-flash verify (2026-08-23 on AI-Thinker ESP32-CAM at `/dev/cu.usbserial-130`, MAC `c8:f0:9e:9d:50:08`)**: erase_flash + flash + provision via softAP `/provision` + boot into normal mode + observe 90 s of post-provision activity.
>
> | FW-13 node | Verified on device? | Evidence |
> |---|---|---|
> | FW-13.1 URI `/cams`, TCP, no MAC, lazy start | ✅ YES | `I (1598) ws: ws_init: ok uri=ws://192.168.1.244:9000/cams disable_auto_reconnect=true transport=TCP (lazy start on IP_EVENT_STA_GOT_IP)` |
> | FW-13.2 hello is first frame after CONNECTED | ⚠️ NOT REACHED | WS CONNECTED event did not fire (TCP connect failed at infra level — see below). Hello + status payload + cadence cannot be observed until AP client isolation is disabled. |
> | FW-13.3 MAC read from eFuse | ⚠️ NOT REACHED | Same as FW-13.2 — the hello frame is the only place this is observable end-to-end. The `esp_read_mac()` API call compiled + linked + the `device_identity_t` was populated by the FW-13.5 status timer init path. |
> | FW-13.4 URL never contains MAC | ✅ YES | The `ws_init` log shows `uri=ws://192.168.1.244:9000/cams` — no MAC substring. The guard would trip if it were present (verified in host tests Pass 11). |
> | FW-13.5 status timer suspend on disconnect | ✅ YES | `E (2628) ws_event: ws_status_timer_stop failed: ESP_ERR_INVALID_STATE` + `I (2638) ws_event: ws disconnected; status timer paused` — timer suspended on DISCONNECTED, exactly per REQ-WS-005 S2. |
> | FW-13.6 8-field status payload | ⚠️ NOT REACHED | Status frames not emitted because CONNECTED event didn't fire. Host tests + design spec verify the 8-field JSON shape. |
>
> **Device-flash caveat (PR body) was correct**: device-flash end-to-end verify requires (a) a WS server on the backend side reachable from the device AND (b) no AP-level client isolation. (a) was satisfied (Python WS server on this Mac at `0.0.0.0:9000/cams`); (b) was NOT — the `Liwaisi Wifi` AP has client isolation enabled. The device TCP-attempt to `192.168.1.244:9000` failed with `esp-tls: [sock=54] connect() error: Host is unreachable` even though both IPs are on the same `192.168.1.0/24` subnet. This is an infrastructure-level constraint, NOT an FW-13 implementation bug.
>
> **Spec compliance** (verify-report pending): 12/12 scenarios across 4 requirements (R-07, R-09, R-17, R-27). 0 CRITICAL. 2 WARNING (both functionally compliant design deviations): (1) `device_identity_t` (NOT `identity_t`) — config.h already defines its own `identity_t`; renaming avoided namespace collision. (2) `ws_init_impl` placeholder pattern in T-13-C — atomic rename in T-13-I avoided duplicate-symbol link error.
>
> **Verify-report verdict**: PARTIAL — full 11-pass host runner GREEN (Pass 1 = 122, Pass 11 fires `url_no_mac`), `idf.py build` GREEN (1,117,136 bytes / 89.7% of 0x130000), `idf.py flash` GREEN on AI-Thinker ESP32-CAM at `/dev/cu.usbserial-130`, and post-provision boot chain GREEN on real silicon through `boot: fw: boot_run ret=ok step=return`. Device-flash end-to-end verify of FW-13.2 hello + FW-13.5 status cadence + FW-13.6 status payload is BLOCKED by AP client isolation on `Liwaisi Wifi` (the device TCP-connect to `192.168.1.244:9000` fails with `Host is unreachable` despite both IPs being on the same `192.168.1.0/24` subnet and the WS server being verified reachable from the Mac itself). 3 of 6 FW-13 leaves verified end-to-end on device (FW-13.1 URI, FW-13.4 URL-no-MAC guard, FW-13.5 status timer suspend on disconnect); 3 leaves (FW-13.2 hello body, FW-13.3 MAC-from-eFuse end-to-end, FW-13.6 status payload content) require a backend host NOT blocked by AP isolation. Host tests cover the remaining surface.
>
> **Known deviations from design** (resolved with documentation, not silent):
>
> 1. `device_identity_t` rename (T-13-C) — config.h has its own `identity_t`; avoided collision. Field shape preserved.
> 2. `ws_init_impl` placeholder (T-13-C) → atomic rename `ws_init` (T-13-I) — avoided duplicate-symbol error with `stub_inits.c::ws_init`.
> 3. `esp_websocket_register_events` (4-arg) instead of `esp_websocket_client_register_events` (5-arg) — discovered in T-13-B against real IDF v1.8.0 header (`managed_components/espressif__esp_websocket_client/include/esp_websocket_client.h`). Per IDF v5.5.3 source, the correct API is 4-arg.
> 4. `esp_websocket_client_close(handle, timeout)` (2-arg) instead of (handle, code, reason, timeout_ms) (4-arg) — real IDF v1.8.0 API.
> 5. `esp_wifi_sta_get_rssi(int *rssi)` (NOT `int32_t *rssi`) — IDF v5.5.3 declares the parameter as `int *`. Cast preserved in caller.
> 6. Factory partition `0x110000` → `0x130000` (T-13-J) — `esp_websocket_client` managed component (~50 KB static lib) + ws/identity component code (~5 KB) pushed post-link firmware.bin to 1,117,136 bytes. Updated partition comment in `firmware/partitions.csv`.
> 7. 5 pre-existing regressions caught by `idf.py build` (T-13-J): Kconfig.projbuild `source` directives resolved from wrong dir; identity.c include typo + missing `esp_mac.h`; ws.c missing `esp_mac.h`; esp_wifi_sta_get_rssi type; Kconfig help text `§` + indentation drop. All fixed in T-13-J commit.
> 8. **XCLK 20 MHz → 10 MHz revert (T-13-M)** — commit `9188c31` bumped `CAMERA_XCLK_FREQ_HZ` 10 MHz → 20 MHz to address a frame-timeout issue. Device-flash verify of FW-13 on 2026-08-23 caught that the 20 MHz value introduced `Detected camera not supported` (ESP_ERR_NOT_SUPPORTED) on AI-Thinker ESP32-CAM at `/dev/cu.usbserial-130` — the sensor probe couldn't identify the OV2640 at 20 MHz. Reverted to the FW-10 verified-working 10 MHz baseline (closure blockquote L976-977 of this doc). The 20 MHz frame-timeout root cause is left for separate investigation (was it the LEDC channel? sccb clock? sensor PLL config?). TODO comment in `firmware/components/camera/camera.c:69`.
> 9. PR LoC ~3,969 (vs pre-approved 1000-line budget + extend-if-needed) — covered by user's `exception-ok` preflight qualifier. The work is tightly coupled per FW-13 Notes L1186 ("hello and status share the same text-frame builder and identity source") so a single PR is the documented choice.
>
> **Device-flash verify (partial, 2026-08-23)**: the AI-Thinker ESP32-CAM at `/dev/cu.usbserial-130` was erased + flashed + provisioned via the FW-05 softAP `/provision` endpoint. Boot reached `boot: fw: boot_run ret=ok step=return`. `ws_init` log line: `ws: ws_init: ok uri=ws://192.168.1.244:9000/cams disable_auto_reconnect=true transport=TCP (lazy start on IP_EVENT_STA_GOT_IP)` — FW-13.1 + FW-13.4 verified on silicon. The WS client fired its lazy start on `IP_EVENT_STA_GOT_IP` (device IP `192.168.1.48`, gateway `192.168.1.1`); the TCP connect attempt failed with `esp-tls: connect() error: Host is unreachable` due to AP client isolation on `Liwaisi Wifi`. The `disable_auto_reconnect=true` setting means the device does NOT retry automatically — it sits in DISCONNECTED until the next IP-up event. The status timer stopped cleanly on DISCONNECTED (FW-13.5 S2 verified). To complete the hello + status frame verify on device, the user must either (a) disable client isolation on `Liwaisi Wifi` and re-flash, or (b) run the WS server on a host NOT on the Wi-Fi broadcast domain. Host tests + `idf.py build` + boot chain on device carry the bulk of the verify evidence.
>
> **FW-14 handoff**: `firmware/components/ws/ws_reconnects.c` returns 0 from `ws_reconnects_get()`. FW-14 replaces this symbol with the real counter impl; status payload at FW-13.6 reads it (currently returns 0, monotonic-or-stable per REQ-WS-006 S3). The `disable_auto_reconnect=true` setting in `ws_init()` ensures the IDF built-in reconnect races FW-14's owner.
>
> **FW-15 handoff**: `firmware/components/ws/ws_text_frame_build_*` API is the carrier for both hello (FW-13.2) and status (FW-13.6). FW-15's binary JPEG frame uses a different path (`esp_websocket_client_send_bin`) and does NOT touch the text-frame builder.
>
> **FW-20 handoff**: `firmware/components/identity/include/identity.h` exports `identity_mac_to_hex_lower` + `device_identity_load` + `device_identity_t`. FW-20 will call `device_identity_load` from the `config` JSON command handler to emit the fresh-hello on identity change.
