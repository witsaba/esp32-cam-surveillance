/* test_softap_guard.c — FW-05.4 strict validation + bite-proof.
 *
 * The /provision handler's strict validation (non-JSON body, JSON
 * missing any of the 4 required keys) lives in
 ...
 *   4. missing_name           — JSON missing name → 400 err="name"
 *   5. missing_description    — JSON missing description → 400 err="description"
 ...
 * provision_post_handler_impl(). Under the stub, the rejection tests
 * FAIL because the handler no longer returns 400 (it now proceeds
 * straight to merge + save + restart). The failure messages contain
 * the literal "validation" so the runner can verify the bite-proof
 * pattern. The well-formed test continues to PASS because its
 * assertions hold under both builds.
 *
 * The `_MESSAGE` form of TEST_ASSERT_* is used so the failure
 * message printed by Unity contains "validation" — required by the
 * bite-proof runner assertion.
 */
    ...
    "[softap][fw-05.4][guard][bite-proof]")
{
    ...
    mock_httpd_req_t *req = drive_provision_guard(body);
    ...
    "[softap][fw-05.4][guard][bite-proof]")
{
    ...
    assert_rejection_with_validation_msg(req, "name");
    ...
    "[softap][fw-05.4][guard][bite-proof]")
{
    ...
    assert_rejection_with_validation_msg(req, "description");
    ...
    "[softap][fw-05.4][green]")
{
    ...
    /* config_save was called (4 str fields + 1 schema_version u8 = 5
     * writes) and esp_restart was called exactly once. */
